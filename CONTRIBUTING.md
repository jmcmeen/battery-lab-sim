...
3. To exercise the chemistry on real hardware, add it to the per-chassis `CHEMISTRY` env override in [`docker-compose.yml`](docker-compose.yml) (or set `CYCLER_NN_CHEMISTRY=<NEW>` in `.env`). The cycler reads `CHEMISTRY` at boot — see [`services/cycler/src/cycler/main.py`](services/cycler/src/cycler/main.py). Any schedule using the new chemistry must set `chemistry: <NEW>` in its YAML; the operator-side contract requires the schedule's chemistry to match the cycler's env (orchestrator-side validation via a chassis-level CHEMISTRY Modbus register is planned as the v0.1.8 schedule-driven runtime follow-up).
